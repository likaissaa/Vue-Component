...
Vue.component('uiselectinput',
{
	template:'\
		<div :style= "uitop">\
			<select  class="form-control" :class="selectclass" v-model="haha">\
				<option v-for="list in lists">{{list.text}}</option>\
			</select>\
			<input type="text" class="form-control" :class="inputclass" v-model="haha" :style="uiinput"/>\
		</div>',
    	props:['lists','haha','selectclass','inputclass'],
    	data:function(){
    		return {
